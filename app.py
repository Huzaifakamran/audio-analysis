...
def convert_audio_to_text(input_path,output_dir,max_size_mb=25):
    ...
    # print('PARTS: ',parts)
    audio_duration = len(audio) / 60000
    ...
    try:
        ...
        if uploaded_file is not None:
            ...
            if st.button("Convert to Text"):
                TranscriptText,audio_duratio = convert_audio_to_text(st.session_state.audio_file,'audios')
                ...
                with duration:
                    st.write(f"The duration of audio file is {audio_duratio} minutes")
                with st.spinner('analysing the conversation to fetch the required details'):    
                    with details:
                        ...
                        The first detail is "Concept", you need to do two things here first one is to understand the concept spoken in the audio.
                        ...
                        which part is said by who (client & salesman). And the second one is to extract nouns spoken over conversation 
                        (and number of times it appears) and differates them between client and salesman.
                             
                        The second detail is "Client Profile", here you also need to two things, first one is to identify the client profile based on
                        the following preferences of purchasing: (Trend,Money,Confort, Fidelity,Security and Pride). And the second one is to 
                        include a percentage distribution of the 6 preferences of client profile for purchaising.
                        Note: 'Pride' and 'Status' is same thing so use one of them.
